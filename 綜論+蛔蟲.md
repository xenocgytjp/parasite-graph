...
| 其他 | 嗜屎性寄生蟲 (Coprozoic parasite) | 與糞便環境相關，常非真正感染 | 糞小桿線蟲 （*Strongyloides stercoralis*）|
...
```mermaid
flowchart TD
    A[Helminths] --> B[Nematodes]
    A --> C[Cestodes]
    A --> D[Trematodes]

    B --> B1[Round worms]
    B --> B2[Body cavity]
    B --> B3[Complete alimentary canal]
    B --> B4[Examples: Ascaris, Trichuris, Enterobius]

    C --> C1[Tapeworms]
    C --> C2[Segmented body]
    C --> C3[No alimentary canal]
    C --> C4[Example: Taenia]

    D --> D1[Flukes]
    D --> D2[Leaf-like body]
    D --> D3[Two suckers]
    D --> D4[Examples: Fasciolopsis, Schistosoma]
```
